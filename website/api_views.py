...
from predictions.prediction_engine import PredictionManager
from .serializers import (
    FarmSerializer,
    CropSerializer,
    FarmBoundaryUploadSerializer,
    FarmMappingStartSerializer,
    FarmMappingFinishSerializer,
)
...
def build_farm_boundary_visual(boundary_points):
    """Build a simple SVG representation of the farm boundary for PDF export."""
    normalized = []
    ...
    lats = [lat for lat, _ in normalized]
    lngs = [lng for _, lng in normalized]
    ...
    height = 320
    padding = 24
    scale_x = (width - 2 * padding) / lng_span
    scale_y = (height - 2 * padding) / lat_span
    ...
    for lat, lng in normalized:
        ...
        transformed.append({'x': round(x, 2), 'y': round(y, 2)})

    polyline = ' '.join(f"{point['x']},{point['y']}" for point in transformed)
    return {
        'width': width,
        'height': height,
        'polyline': polyline,
        'points': transformed,
        'viewbox': {'min_lat': min_lat, 'max_lat': max_lat, 'min_lng': min_lng, 'max_lng': max_lng},
    }


def build_static_map_url(boundary_points):
    """Build a static map image URL covering the full farm boundary."""
    normalized = []
    for point in boundary_points or []:
        lat = point.get('lat', point.get('latitude'))
        lng = point.get('lng', point.get('lon', point.get('longitude')))
    ...
    lng_margin = max((max_lng - min_lng) * 0.2, 0.0005)

    bbox = ','.join([
        str(round(min_lng - lng_margin, 7)),
        str(round(min_lat - lat_margin, 7)),
        str(round(max_lng + lng_margin, 7)),
        str(round(max_lat + lat_margin, 7)),
    ])
    return (
    ...
    def create(self, request, *args, **kwargs):
        if self.get_queryset().count() >= 2:
            return Response(
                {
                    'status': 'error',
                    'message': 'A user can only have up to 2 farms.'
                },
...
class FarmBoundaryUploadAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            return Response({'error': 'Farm not found for this user'}, status=status.HTTP_404_NOT_FOUND)

        serializer = FarmBoundaryUploadSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        boundary_points = serializer.validated_data['boundary_points']
        farm.boundary_points = boundary_points
        if 'gps_accuracy_meters' in serializer.validated_data:
            farm.gps_accuracy_meters = serializer.validated_data['gps_accuracy_meters']
        if 'mapping_method' in serializer.validated_data:
            farm.mapping_method = serializer.validated_data['mapping_method']

        verification = farm.verify_boundary() if boundary_points else None
        farm.save()
        if boundary_points:
            verification = farm.verify_boundary()
            farm.save(update_fields=[
                'gps_latitude',
                'gps_longitude',
                'gps_accuracy_meters',
                'boundary_points',
                'mapped_area_hectares',
                'mapping_method',
                'verification_status',
                'verification_notes',
                'last_mapped_at',
...
class FarmMappingStartAPIView(APIView):
    """Start a mobile farm mapping session."""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        serializer = FarmMappingStartSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        farm.start_mapping_session(
            started_at=serializer.validated_data.get('started_at'),
            mapping_method=serializer.validated_data.get('mapping_method'),
            gps_accuracy_meters=serializer.validated_data.get('gps_accuracy_meters'),
        )
        farm.save(update_fields=[
            'mapping_session_status',
            'mapping_started_at',
            'mapping_finished_at',
...
class FarmMappingFinishAPIView(APIView):
    """Finish a mobile farm mapping session, save boundary, and compute stats."""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        serializer = FarmMappingFinishSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        farm.boundary_points = serializer.validated_data['boundary_points']
        if 'gps_accuracy_meters' in serializer.validated_data:
            farm.gps_accuracy_meters = serializer.validated_data['gps_accuracy_meters']
        if 'mapping_method' in serializer.validated_data:
            farm.mapping_method = serializer.validated_data['mapping_method']
        ...
        farm.save()
        farm.finish_mapping_session(finished_at=serializer.validated_data.get('finished_at'))
        verification = farm.verify_boundary()
        farm.save(update_fields=[
            'gps_latitude',
            'gps_longitude',
        ...
        return Response({
            'status': 'success',
            'message': 'Mapping session finished',
            'farm': FarmSerializer(farm).data,
            'verification': verification,
        })


class FarmBoundaryVerificationAPIView(APIView):
    """Return the latest automated verification status for a farm boundary."""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request, pk):
        ...
        if not farm.has_boundary_map:
            return Response(
                {
                    'status': 'missing_boundary',
                    'message': 'Farm does not have a mapped boundary yet.',
                    'farm': FarmSerializer(farm).data,
                },
        ...
        verification = farm.verify_boundary()
        farm.save(update_fields=['verification_status', 'verification_notes', 'updated_at'])
        return Response({
            'status': 'success',
            'farm_id': farm.id,
            'verification': verification,
...
class FarmBoundaryPdfAPIView(APIView):
    """Export a mapped farm summary as PDF."""
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request, pk):
        if HTML is None:
            return Response({'error': 'PDF export is unavailable on this server'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        ...
        if not farm.has_boundary_map:
            return Response({'error': 'Farm does not have a mapped boundary yet'}, status=status.HTTP_400_BAD_REQUEST)

        verification = farm.verify_boundary()
        farm.save(update_fields=['verification_status', 'verification_notes', 'updated_at'])

        html = render_to_string('website/farm_boundary_pdf.html', {
            'farm': farm,
            'verification': verification,
            'boundary_points': farm.boundary_points,
            'boundary_visual': build_farm_boundary_visual(farm.boundary_points),
            'static_map_url': build_static_map_url(farm.boundary_points),
            'generated_at': timezone.now(),
            'owner_name': farm.owner.get_full_name(),
        })
        try:
            pdf_bytes = HTML(string=html, base_url=request.build_absolute_uri('/')).write_pdf()
        except Exception as exc:
            return Response(
                {'error': f'PDF export is configured but unavailable on this server: {exc}'},
                status=status.HTTP_503_SERVICE_UNAVAILABLE
            )

        response = HttpResponse(pdf_bytes, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="farm_{farm.id}_boundary_map.pdf"'
        return response
...
def build_monthly_rainfall_plan(forecasts):
    ...
    total_rainfall = round(sum(float(item.expected_rainfall) for item in forecasts), 2)
    rainy_days = sum(1 for item in forecasts if float(item.expected_rainfall) >= 2)
    heavy_rain_days = sum(1 for item in forecasts if float(item.expected_rainfall) >= 10)
    dry_days = sum(1 for item in forecasts if float(item.expected_rainfall) < 1)
    avg_probability = round(
        sum(float(item.probability) for item in forecasts) / len(forecasts),
        1
    )

    weekly_breakdown = []
    for index in range(0, len(forecasts), 7):
        week_slice = forecasts[index:index + 7]
        week_total = round(sum(float(item.expected_rainfall) for item in week_slice), 2)
        week_probability = round(
            sum(float(item.probability) for item in week_slice) / len(week_slice),
            1
        )

        if week_total >= 50:
            risk_level = 'high'
            advice = 'Expect a wet week. Prioritize drainage, field access, and erosion control.'
        elif week_total >= 20:
            risk_level = 'medium'
            advice = 'Moderate rainfall expected. Good for planting and fertilizer timing if the soil is workable.'
        else:
            risk_level = 'low'
            advice = 'Likely dry week. Plan irrigation, mulch, and moisture conservation.'

        weekly_breakdown.append({
            'week': len(weekly_breakdown) + 1,
            'start_date': week_slice[0].forecast_date.isoformat(),
            'end_date': week_slice[-1].forecast_date.isoformat(),
            'total_rainfall_mm': week_total,
            'average_probability_percent': week_probability,
            'risk_level': risk_level,
            'planning_advice': advice,
        })

    if total_rainfall >= 120:
        monthly_outlook = 'wet'
        recommendation = 'Plan for strong rainfall this month. Check drainage, reduce waterlogging risk, and be careful with fertilizer wash-off.'
    elif total_rainfall >= 60:
        monthly_outlook = 'balanced'
        recommendation = 'Rainfall looks fairly supportive this month. Planting and routine field work can be timed around the wetter weeks.'
    else:
        monthly_outlook = 'dry'
        recommendation = 'Rainfall looks limited this month. Prioritize irrigation planning, water storage, and drought-tolerant scheduling.'

    return {
        'forecast_window_days': len(forecasts),
        'forecast_start_date': forecasts[0].forecast_date.isoformat(),
        'forecast_end_date': forecasts[-1].forecast_date.isoformat(),
        'predicted_total_rainfall_mm': total_rainfall,
        'average_daily_rainfall_mm': round(total_rainfall / len(forecasts), 2),
        'average_rain_probability_percent': avg_probability,
        ...
        'daily_forecast': [
            {
                ...
                'expected_rainfall_mm': round(float(forecast.expected_rainfall), 2),
                'probability_percent': round(float(forecast.probability), 1),
...
def fetch_weather_forecast_data(lat, lon, days_requested=5):
    """Fetch and format OpenWeather forecast data for API responses."""
    ...
    data['current_weather'] = {
        ...
        'advice': get_farmer_advice(current),
    }

    max_days = min(days_requested, len(weather_data['daily']) - 1)
...
def monthly_rainfall_plan(request):
    ...
    days_requested = int(request.query_params.get('days', 30))
    days_requested = max(14, min(days_requested, 30))
    ...
    if farm_id:
        try:
            farm = farms.get(pk=farm_id)
        except Farm.DoesNotExist:
            return Response(
                {'error': 'Farm not found for this user'},
                status=status.HTTP_404_NOT_FOUND
            )
    else:
        farm = farms.first()

    historical_points = farm.weather_data.count()
    if historical_points < 10:
        return Response(
            {
                'error': 'Not enough historical rainfall data to build a monthly forecast',
                'farm_id': farm.id,
                'historical_records': historical_points,
    ...
    success = manager.forecast_rainfall(farm.id, days=days_requested)
    if not success:
        return Response(
            {
                'error': 'Unable to generate the monthly rainfall forecast',
                'farm_id': farm.id,
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
    ...
    forecasts = list(
        RainForecast.objects.filter(
            farm=farm,
            forecast_date__gte=today,
        ).order_by('forecast_date')[:days_requested]
    )

    if not forecasts:
        return Response(
            {
                'error': 'Forecast generation completed without forecast records',
                'farm_id': farm.id,
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )

    plan = build_monthly_rainfall_plan(forecasts)
    return Response({
        'status': 'success',
        'farm': {
            'id': farm.id,
        ...
        'historical_records_used': historical_points,
        'plan': plan,
    })
