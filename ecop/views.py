...
<<<<<<< HEAD
            ...
            farmer_commitments = []
            member_ids = set(
                group.members.filter(is_active=True).values_list('user_id', flat=True)
            )
            
            for farmer_data in serializer.validated_data['farmer_commitments']:
                farmer_id = int(farmer_data['farmer_id'])
                if farmer_id not in member_ids:
                    return Response({
                        'status': 'error',
                        'message': f'Farmer {farmer_id} is not an active member of this group.'
                    }, status=status.HTTP_400_BAD_REQUEST)

                farmer = get_object_or_404(User, id=farmer_id)
                volume = Decimal(str(farmer_data['volume']))
...
<<<<<<< HEAD
        platform_stats = {
            ...
            'total_volume_committed': float(
                EcopCommitment.objects.aggregate(total=Sum('total_volume'))['total'] or 0
            ),
            'total_transactions': EcopCommitment.objects.filter(status__in=['matched', 'paid']).count(),
        ...
        group_stats = [{
            ...
            'total_volume': float(group.commitments.aggregate(total=Sum('total_volume'))['total'] or 0),
            'total_transactions': group.commitments.filter(status__in=['matched', 'paid']).count(),
            'success_rate': self._calculate_group_success_rate(group),
        } for group in groups]
    ...
    def _calculate_total_payments(self):
        paid_commitments = EcopFarmerCommitment.objects.select_related('commitment').filter(
            status='paid'
        )
        total = 0
        for farmer_commitment in paid_commitments:
            if farmer_commitment.payment_amount is not None:
                total += float(farmer_commitment.payment_amount)
                continue
            price = farmer_commitment.commitment.agreed_price or farmer_commitment.commitment.target_price or 0
            total += float(price) * float(farmer_commitment.volume)
        return round(total, 2)

    def _calculate_group_success_rate(self, group):
        total = group.commitments.count()
        if total == 0:
            return 0.0
