...
class EcopGroup(models.Model):
    ...
    def clean(self):
        if EcopGroup.objects.filter(group_name__iexact=self.group_name).exclude(pk=self.pk).exists():
...
class EcopCommitment(models.Model):
    ...
    crop = models.CharField(max_length=100, help_text="Type of crop being committed")
    ...
    def match(self, buyer, price_per_kg):
        if self.status in {"active", "confirmed"}:
            ...
            self.save(update_fields=["status", "buyer", "agreed_price", "matched_at"])
            return True
        return False
